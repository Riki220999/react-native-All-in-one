...
import {AUTHSUCCESS, AUTHFAILED, LOGGEDIN} from '../ConfigAuth';
import _ from '../../../lang';
...
class SignInAuth extends Component {
    ...
    static getDerivedStateFromProps(nextProps, prevState) {
        if (nextProps.action !== prevState.action) {
            ...
            return {action: nextProps.action, err: nextErr, res: nextRes};
        }
    ...
    async componentDidUpdate(prevProps, prevState) {
        if (prevState.action !== this.state.action) {
            if (this.state.action === AUTHFAILED) {
                Alert.alert('Error', this.state.err, [
                    {
                        text: 'Ok',
                        onPress: () => {
...
const mapStateToProps = state => (
    {fetch: state.auth.fetch, res: state.auth.res, err: state.auth.err, action: state.auth.action}
);
